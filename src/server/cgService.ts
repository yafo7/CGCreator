  ...
  async refine(id: string, revision: number, prompt: string, targetId?: string) {
    ...
    const project = await this.run(id, 'refining', '生成局部意图修改', () => this.store.transaction(id, revision, async (project) => {
      ...
      if (selected && ['慢', '慢点', '再慢', '快', '快点', '特写', '近景', '中景', '远景', '全景'].includes(simple)) {
        ...
        else operations = [{ type: 'shot.update', id: selected.id, patch: { camera: { framing: ['特写', '近景'].includes(simple) ? 'close-up' : simple === '中景' ? 'medium' : 'wide' } } }];
      } else {
...
Entity: {id,name,kind:'actor'|'prop',objectId?,assetId?,startAnchorId?,description?,height?}. Anchor: exact supplied object. Shots are sequential {id,name,purpose,duration,camera,subtitle?}. Camera is semantic {movement:'static'|'dolly'|'tracking'|'orbit',framing:'wide'|'medium'|'close-up'|'over-shoulder',subjectId,secondaryId?,side?:'left'|'right',lensMm?,distance?,height?,azimuth?}. Use physical lens 12..200mm; azimuth radians. Framing and target entity drive camera placement at scale. Over-shoulder requires two distinct subjects. Favor stable composition and restrained movement; camera movement cannot own actor motion.
